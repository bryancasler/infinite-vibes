...
import type { InspirationPreset } from '../types.js';
...
export class InspirationPanel extends LitElement {
  ...
  @state() private presets: InspirationPreset[] = [];

  private orchestrator = getOrchestrator();
  private inspirationService = getInspirationService();
  ...
  private handleApplyPreset(presetId: string): void {
    this.orchestrator.applyPreset(presetId);
  }

  private handleAddRandom(): void {
    this.orchestrator.addRandomPrompt();
  }

  private handleAddComplement(): void {
    this.orchestrator.addComplementPrompt();
  }

  private handleRandomPreset(): void {
            ...
            (preset) => html`
              ...
              >
                ...
                <div class="preset-prompts">
                  ${preset.prompts.slice(0, 4).map(
                    (p) => html`
                      <span
                        class="preset-prompt-tag"
                        style="background: ${withAlpha(
                          p.color,
                          0.3
                        )}; border: 1px solid ${withAlpha(p.color, 0.5)}"
                      >
                        ${p.text}
                      </span>
                    `
