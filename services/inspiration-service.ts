...
export class InspirationService {
  ...
  private presets: InspirationPreset[] = [...INSPIRATION_PRESETS];
  ...
  applyPreset(presetId: string): {
    ...
    const prompts: Prompt[] = preset.prompts.map((p) => ({
      id: generateId(),
      text: p.text,
      weight: p.weight,
      color: p.color,
   ...
   */
  generateComplementPrompt(
    existingPrompts: Prompt[],
    existingColors: string[] = []
  ): Prompt {
    // Analyze existing prompts
    const existingTexts = existingPrompts.map((p) => p.text.toLowerCase());
    const hasBass = existingTexts.some(
      (t) => t.includes('bass') || t.includes('sub')
    );
    const hasDrums = existingTexts.some(
      (t) =>
        t.includes('drum') ||
        t.includes('kick') ||
        t.includes('hi-hat') ||
    ...
    const hasMelody = existingTexts.some(
      (t) =>
        t.includes('melody') ||
        t.includes('synth') ||
        t.includes('piano') ||
    ...
    const hasAtmosphere = existingTexts.some(
      (t) =>
        t.includes('ambient') ||
        t.includes('pad') ||
...
export const getInspirationService = (): InspirationService =>
  InspirationService.getInstance();
