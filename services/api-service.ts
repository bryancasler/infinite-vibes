  ...
  async sendPromptUpdate(
    ...
    if (settings) {
      ...
      if (settings.density !== undefined) {
        settingsParts.push(
          `Density: ${settings.density < 0.3 ? 'sparse' : settings.density > 0.7 ? 'dense' : 'medium'}`
        );
      }
      if (settings.brightness !== undefined) {
        settingsParts.push(
          `Brightness: ${settings.brightness < 0.3 ? 'dark' : settings.brightness > 0.7 ? 'bright' : 'neutral'}`
        );
      }
...
export const getApiService = (): ApiService => ApiService.getInstance();
