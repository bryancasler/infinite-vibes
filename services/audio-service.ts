  ...
  queueAudioChunk(base64Data: string): void {
    ...
    try {
      ...
      const fadeSamples = Math.floor(
        AUDIO_CONFIG.FADE_DURATION * AUDIO_CONFIG.SAMPLE_RATE
      );
      const processedData =
        this.audioQueue.length === 0 && this.scheduledSources.length === 0
    ...
    try {
      ...
      while (this.audioQueue.length > 0) {
        ...
        source.onended = () => {
          ...
          this.totalBufferedDuration = Math.max(
            0,
            this.totalBufferedDuration - buffer.duration
          );
          this.bufferLevel = Math.min(1, this.totalBufferedDuration / 5);
...
export const getAudioService = (): AudioService => AudioService.getInstance();
