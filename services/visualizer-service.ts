...
import type { VisualizationMode, VisualizerConfig, Particle } from '../types.js';
import { DEFAULT_VISUALIZER_CONFIG, COLOR_SCHEMES, NEON_COLORS } from '../utils/constants.js';
import { getAudioService } from './audio-service.js';
import { withAlpha, interpolateColor, hexToRgb } from '../utils/color.js';
 ...
 */
class GradientPulseVisualization implements VisualizationStrategy {
  private phase = 0;
 ...
 */
class ParticleVisualization implements VisualizationStrategy {
  ...
  render(
    ...
    const bassAvg =
      frequencyData.slice(0, 10).reduce((a, b) => a + b, 0) / 10 / 255;
    const trebleAvg =
      frequencyData.slice(100, 150).reduce((a, b) => a + b, 0) / 50 / 255;
 ...
 */
class SpectrogramVisualization implements VisualizationStrategy {
 ...
 */
class WarpVisualization implements VisualizationStrategy {
  private stars: Array<{ x: number; y: number; z: number; color: string }> = [];
  private initialized = false;

  private initStars(width: number, height: number, config: VisualizerConfig): void {
    const colors = COLOR_SCHEMES[config.colorScheme];
    this.stars = [];
  ...
  render(
    ...
    const bassAvg =
      frequencyData.slice(0, 20).reduce((a, b) => a + b, 0) / 20 / 255;
 ...
 */
class AlbumArtVisualization implements VisualizationStrategy {
  private smoothedVolume = 0;
  private phase = 0;
...
export class VisualizerService {
  private static instance: VisualizerService;
  ...
  private strategies: Map<VisualizationMode, VisualizationStrategy> = new Map();
  private currentStrategy: VisualizationStrategy | null = null;
...
export const getVisualizerService = (): VisualizerService =>
  VisualizerService.getInstance();
