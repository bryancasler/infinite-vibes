...
import type { ChatAction, ChatResponse, Prompt, ChatMessage } from '../types.js';
...
export class ChatService {
  ...
  async initialize(): Promise<boolean> {
    ...
    try {
      ...
      this.chatSession = (this.model as {
        startChat: (config: { history: unknown[] }) => unknown;
      }).startChat({
        history: [],
      });
  ...
  async sendMessage(
    ...
    try {
      ...
      const chatSession = this.chatSession as {
      ...
      try {
        ...
        if (Array.isArray(rawActions)) {
          actions = rawActions.map((action) => {
            ...
            if (
              (action.action === 'update' || action.action === 'remove') &&
              action.prompt &&
              !action.promptId
            ) {
              const match = this.findMatchingPrompt(
                action.prompt,
                currentPrompts
              );
              if (match) {
  ...
  private findMatchingPrompt(
    ...
    const exactMatch = prompts.find(
      (p) => p.text.toLowerCase() === search
    );
    if (exactMatch) return exactMatch;

    // Try partial match
    const partialMatch = prompts.find(
      (p) =>
    ...
    let bestMatch: Prompt | null = null;
    let bestScore = 0;
   ...
   */
  async generateSuggestions(currentPrompts: Prompt[]): Promise<string[]> {
    if (!this.isInitialized) {
      const initialized = await this.initialize();
    ...
    try {
      ...
      const result = await model.generateContent(
        `Given a music mix with: ${promptList}
...
export const getChatService = (): ChatService => ChatService.getInstance();
