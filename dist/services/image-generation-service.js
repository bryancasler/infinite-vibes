...
export class ImageGenerationService {
     ...
     */
    async generateAlbumArt(prompts, config) {
...
Describe the visual elements, colors, composition, and mood. Keep it under 100 words.`,
     ...
     */
    buildImagePrompt(prompts, config) {
        ...
        if (enabledPrompts.length > 0) {
            const moods = enabledPrompts.map((p) => {
                ...
                if (text.includes('dark') || text.includes('deep'))
                    return 'dark, moody';
                if (text.includes('bright') || text.includes('happy'))
                    return 'bright, vibrant';
                if (text.includes('chill') || text.includes('ambient'))
                    return 'calm, serene';
     ...
     */
    createPlaceholderDataUrl(prompts, config) {
        ...
        // Background gradient
        const enabledPrompts = prompts.filter((p) => p.enabled);
        const colors = enabledPrompts.length > 0
...
export const getImageGenerationService = () => ImageGenerationService.getInstance();
